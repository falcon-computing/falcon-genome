    ...
    boost::program_options::options_description &opt_desc)
{
  ...
  std::vector<std::string> input_files2_ ;
  int contig_range;
  if (boost::filesystem::is_directory(input_path)) {
    get_input_list(input_path, input_files2_, ".*/part*-[0-9].*.*", true);
    if (input_files2_.size()>get_config<int>("gatk.ncontigs")) {
      contig_range=input_files2_.size();
    }
    else {
      contig_range=get_config<int>("gatk.ncontigs");
    }
    //LOG(INFO) << input_files2_.size() << "\n";
  }
  else{
    for (int n = 0; n < get_config<int>("gatk.ncontigs"); n++) {
      input_files2_.push_back(input_path);
    };
  }
  ...
  if (!intv_list.empty()) {
    ...
    if (contig_range==get_config<int>("gatk.ncontigs")){
      intv_paths = init_contig_intv(ref_path);
    } else {      
      get_input_list(input_path, intv_paths, ".*/*/intervals/interval*_[0-9].*.bed", true);
      std::sort(intv_paths.begin(), intv_paths.end());
      LOG(INFO) << "I am here";
    }
  ...
  //for (int contig = 0; contig < contig_range; contig++) {
  //for (auto x : input_files2_) {

  for (int k=0 ; k<contig_range-1; k++) {
   ...
   //}
    ...
    LOG(INFO) << input_files2_[k] << "  " << intv_paths[k] << " " << contig_range << "\n";    
    std::string file_ext = "vcf";
    if (!flag_vcf) {
      file_ext = "g." + file_ext;
    }
 
    std::string output_file = get_contig_fname2(output_dir, k, contig_range, file_ext);
