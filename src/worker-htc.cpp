    ...
    boost::program_options::options_description &opt_desc)
{
  ...
  for (int contig = 0; contig < get_config<int>("gatk.ncontigs"); contig++) {
    ...
    if (boost::filesystem::is_directory(input_path)) {
      ...
      input_bed = get_contig_fname(input_path, contig, "bed");
    ...
    //LOG(INFO) << input_file << " " << input_bed;
    if (boost::filesystem::exists(input_bed) ) {
      if(!intv_list.empty()){
         intv_paths[0] = input_bed + " -L " + intv_paths[0];
      }
      else {
	intv_paths[0] = input_bed;
      }
    }

    LOG(INFO) << input_file << " " << intv_paths[0];
