...
Executor* create_executor(std::string job_name, std::vector<std::string> stage_levels, std::string sample_id, int num_workers) {
  Executor* executor = new Executor(job_name, stage_levels, sample_id, num_workers);
  g_executor = executor;
