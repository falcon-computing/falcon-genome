...
class SampleSheet{
   ...
   public:
     ...
     bool is_file();
     bool is_dir();
     map<string, vector<subject> > extract_data_from_file();
     map<string, vector<subject> > extract_data_from_folder();
};   

SampleSheet::SampleSheet(string a){fname=a;};
string SampleSheet::get_fname(){return fname;};
...
map<string, vector<subject> > SampleSheet::extract_data_from_file(){
  ...
  string sampleName, read1, read2, rg, platform, library_id; 
  subject sampleInfo;

  int check_fields;
  string grab_line_info;
...
map<string, vector<subject> > SampleSheet::extract_data_from_folder(){
  ...
  string target="1.fastq.gz";
  string task="2.fastq.gz";
  string delimiter="_";
  string new_delimiter=" ";
  string temp_id, sample_id;
