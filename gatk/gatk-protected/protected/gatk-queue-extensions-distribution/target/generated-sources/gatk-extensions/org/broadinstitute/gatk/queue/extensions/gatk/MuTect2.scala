...
@Argument(fullName="m2debug", shortName="m2debug", doc="Print out very verbose M2 debug information", required=false, exclusiveOf="", validation="")
var m2debug: Boolean = _

/** Enable artifact detection for creating panels of normals */
@Argument(fullName="artifact_detection_mode", shortName="", doc="Enable artifact detection for creating panels of normals", required=false, exclusiveOf="", validation="")
...
@Argument(fullName="initial_normal_lod", shortName="", doc="Initial LOD threshold for calling normal variant", required=false, exclusiveOf="", validation="")
var initial_normal_lod: Option[Double] = None

/** Format string for initial_normal_lod */
@Argument(fullName="initial_normal_lodFormat", shortName="", doc="Format string for initial_normal_lod", required=false, exclusiveOf="", validation="")
...
@Argument(fullName="debug_read_name", shortName="", doc="trace this read name through the calling process", required=false, exclusiveOf="", validation="")
var debug_read_name: String = _

/** Set an alternate MQ threshold for debugging */
@Argument(fullName="MQ_filtering_level", shortName="MQthreshold", doc="Set an alternate MQ threshold for debugging", required=false, exclusiveOf="", validation="")
...
@Input(fullName="normal_panel", shortName="PON", doc="VCF file of sites observed in normal", required=false, exclusiveOf="", validation="")
...
@Argument(fullName="keepRG", shortName="keepRG", doc="Only use read from this read group when making calls (but use all reads to build the assembly)", required=false, exclusiveOf="", validation="")
var keepRG: String = _
...
@Argument(fullName="errorCorrectReads", shortName="errorCorrectReads", doc="Use an exploratory algorithm to error correct the kmers used during assembly.  May cause fundamental problems with the assembly graph itself", required=false, exclusiveOf="", validation="")
...
@Argument(fullName="captureAssemblyFailureBAM", shortName="captureAssemblyFailureBAM", doc="If specified, we will write a BAM called assemblyFailure.bam capturing all of the reads that were in the active region when the assembler failed for any reason", required=false, exclusiveOf="", validation="")
var captureAssemblyFailureBAM: Boolean = _

/** If specified, we will not analyze soft clipped bases in the reads */
@Argument(fullName="dontUseSoftClippedBases", shortName="dontUseSoftClippedBases", doc="If specified, we will not analyze soft clipped bases in the reads", required=false, exclusiveOf="", validation="")
...
@Argument(fullName="justDetermineActiveRegions", shortName="justDetermineActiveRegions", doc="If specified, the HC won't actually do any assembly or calling, it'll just run the upfront active region determination code.  Useful for benchmarking and scalability testing", required=false, exclusiveOf="", validation="")
