...
print_help() {
  if [[ $pass_args != YES && $1 != gatk ]];then
    ...
    $JAVA -d64 -jar $GATK -T UnifiedGenotyper --help
  fi
}
...
for contig in $contig_list; do
  $DIR/../fcs-sh "$DIR/unifiedGT_contig.sh \
      $ref_fasta \
      $PWD/.fcs-genome/intv_$nparts/intv${contig}.list \
      ${contig_bam[$contig]} \
      ${contig_vcf[$contig]} \
      $contig \
      $additional_args" 2> $log_dir/unifiedGT_contig${contig}.log &
...
log_file=$log_dir/unifiedGT.log
rm -f $log_file
is_error=0
for contig in $contig_list; do
  ...
  contig_log=$log_dir/unifiedGT_contig${contig}.log
